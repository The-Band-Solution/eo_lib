...
def test_create_person(service, mock_repo):
    ...
    ident = "ID123"
    bday = date(1990, 1, 1)
    expected_person = Person(name=name, emails=emails, identification_id=ident, birthday=bday, id=1)
...
def test_update_person(service, mock_repo):
    ...
    mock_repo.update.return_value = Person(name="New", emails=["old@test.com"], id=1)
...
def test_delete_person_not_found(service, mock_repo):
