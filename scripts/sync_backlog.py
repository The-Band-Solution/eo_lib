...
def generate_markdown(issues):
    ...
    md += (
        "This document is automatically synchronized with GitHub Issues. Last updated: "
    )
    md += (
        subprocess.run(
    ...
    for i in issues:
        ...
        sprints = [
            l["name"].replace("Sprint: ", "")
            for l in i.get("labels", [])
            if l["name"].startswith("Sprint:")
        ]
        sprint_str = ", ".join(sprints) if sprints else "-"
        # Milestone
        milestone = (
            i.get("milestone", {}).get("title", "-") if i.get("milestone") else "-"
        )
        # Assignees
    ...
    for i in issues:
        sprints = [
            l["name"] for l in i.get("labels", []) if l["name"].startswith("Sprint:")
        ]
        if not sprints:
            if "No Sprint" not in sprints_map:
                sprints_map["No Sprint"] = []
            sprints_map["No Sprint"].append(i)
        for s in sprints:
            if s not in sprints_map:
                sprints_map[s] = []
    ...
    for i in issues:
        m = (
            i.get("milestone", {}).get("title", "Backlog / No Milestone")
            if i.get("milestone")
            else "Backlog / No Milestone"
        )
        if m not in milestones_map:
            milestones_map[m] = []
    ...
    for i in issues:
        ...
        if i.get("body"):
            body_snippet = (
                i["body"][:300] + "..." if len(i["body"]) > 300 else i["body"]
            )
            md += f"\n**Description**:\n{body_snippet}\n"
        md += "\n---\n\n"
